#!/usr/bin/env python3
"""
postprocess_Integrated_final.py (Dynamics-Only SOTA + FOGM DOB + Batch Mode)
====================================================================
[주요 업데이트 내역]
1. 모달/필터 튜닝: Dynamics 단일화, Low Q / High R, Fixed FOGM DOB 적용 완료
2. 관측값 재구성: [Pos, Vel, Gyr] 3채널 NIS (상수 C=1.0)
3. 시각화 개선: 2D/3D 위치 추종, FOGM Ext Force 등 7종 정밀 플롯 지원
4. [신규] Batch Mode 지원: 지정된 폴더의 모든 .npz를 순차 처리하여 
   개별 플롯 저장, 개별 RL 데이터 저장, 그리고 전체 통합 RL 데이터셋 병합 수행

   python3 UKF.py --mode single --input data_raw/ep0010_circle_S-fdi_ramp.npz

   python3 UKF.py --mode batch --input-dir data_raw --output my_final_rl_dataset
====================================================================
...
# ======================================================================
#  1. Dynamics-Centric UKF (15 states) + FOGM DOB
# ======================================================================
class DynamicsUKF:
    def __init__(self, dt=0.02, calib=None):
        ...
        self.tau_ext = 2.0 
        self.Q = np.diag([1e-4]*3 + [1e-4]*3 + [1e-4]*3 + [1e-4]*3 + [5e-2]*3)
        self.R = np.diag([2.0]*3 + [1.0]*3 + [0.5] + [0.5]*2 + [1.0]*3)
...
def plot_ext_force(ext_force, label_s, label_h, label_d, title, save_path):
    fig, axes = plt.subplots(3, 1, figsize=(12, 8), sharex=True)
    fig.suptitle(title, fontsize=16, fontweight='bold')
    labels = ['Ext Force N (X axis)', 'Ext Force E (Y axis)', 'Ext Force D (Z axis)']
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c']
    for i in range(3):
        axes[i].plot(ext_force[:, i], color=colors[i], lw=1.2, label=f'Estimated {labels[i]}')
        draw_label_spans(axes[i], label_s, label_h, label_d); axes[i].set_ylabel('Force'); axes[i].grid(True, alpha=0.3); axes[i].legend(loc='upper right')
    axes[2].set_xlabel('Steps')
    plt.tight_layout(); plt.savefig(save_path, dpi=150); plt.close()
...
def generate_all_plots(res, lbl_s, lbl_h, lbl_d, p, out_dir):
    ...
    plot_ext_force(res['ext_force'], lbl_s, lbl_h, lbl_d, f"Estimated External Force (FOGM DOB) ({p})", os.path.join(out_dir, 'ext_force_dob.png'))
...
def process_episode(data, calib):
    ...
    res = { 
        ...
        'est_pos_d_enu': np.zeros((N, 3)), 'res_12ch': np.zeros((N, 12)), 'ext_force': np.zeros((N, 3)),
        'setpoint_pos': sp_pos
    }
    ...
    for k in range(N):
        ...
        res['res_12ch'][k] = rd
        res['ext_force'][k] = ukf_dyn.x[12:15].copy()
...
def mode_batch(args):
    ...
    # 1. 지정된 폴더 안의 모든 .npz 파일을 검색 (기본값: data_raw/)
